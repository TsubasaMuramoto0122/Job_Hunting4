...
public:
	...
	// オブジェクト生成
	static CModel *Create(D3DXVECTOR3 pos, D3DXVECTOR3 rot, D3DXVECTOR3 scale, int nXType, bool bCollision = true, bool bPush = true);
	...
	void SetNumParent(int Parent)				{ m_nNumParent = Parent; }		// 親番号設定											// モデル頂点のスケール変更
	...
	bool		GetPlayerHitBool()		{ return m_bHitPlayer; }			// プレイヤーが当たった情報を取得
	bool		GetHitFromLeft()		{ return m_bHitLeft; }				// 左からの当たり判定情報を取得
...
private:
	...
	bool						m_bHit;						// 当たったか
	bool						m_bHitPlayer;				// プレイヤー当たり判定
	bool						m_bHitLeft;					// 左から当たったか
	bool						m_bPush;					// 押し出すか(当たった判定のみ欲しい場合)
};
#endif