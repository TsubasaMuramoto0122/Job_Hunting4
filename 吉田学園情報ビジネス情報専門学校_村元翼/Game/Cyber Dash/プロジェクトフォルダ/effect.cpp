...
// 生成処理
//=============================================================================
CEffect *CEffect::Create(D3DXVECTOR3 pos, D3DXVECTOR3 size, D3DXCOLOR col, float fSub, int nType, const char* TexName)
{
	// インスタンス生成
	CEffect *pEffect = nullptr;

	if (pEffect = new CEffect(OBJTYPE_EFFECT))
	{
		pEffect->CScene::SetCol(col);															// カラーの設定
		pEffect->m_fSub = fSub;																	// 減少量の設定
		pEffect->Init(pos, size);																// 初期化
		pEffect->BindTexture(CManager::GetInstance()->GetTexture()->GetTexture(TexName));		// テクスチャの取得
		...
		case EFFECT_FALL:		// 落ちていくエフェクト
			pEffect->m_pControl = CControlRise::Create(true);
			break;
		}
	}
...
void CEffect::Draw(void)
{
	...
	// ポリゴンをカメラに対して正面に向ける
	D3DXMatrixInverse(&m_mtxWorld, NULL, &m_mtxView);  //逆行列を求める
	m_mtxWorld._41 = 0.0f;
	m_mtxWorld._42 = 0.0f;
	m_mtxWorld._43 = 0.0f;
